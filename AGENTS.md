...
96. A `needs_revision` or `unscorable` background preview may continue to V0.7 review. The exact optimization review must carry its quality status, primary findings, decorative warnings, limitations, and standard-workflow recommendation; only the user’s exact V0.7 plan-hash approval may authorize packaging.
...
6. Show that review and plan SHA-256 to the user. Stop without optimization until the user chooses `approve`, `revise_profile`, or `cancel`.
7. On `revise_profile`, explicitly update the profile and start a fresh preflight/review run. On approval, persist one exact, hash-bound, single-use `optimization_approval.json`.
