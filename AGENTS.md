...
17. `production/autonomy/<session-id>/` — opt-in AQ root/profile/budget, workflow-owned candidates, policy authorizations, promotions, attempts, transitions, package repairs, reconstructed state, and terminal evidence; never evidence of synthesized user approval.
18. `reference_evidence/runs/<run-id>/` and `reports/integrated_quality/runs/<run-id>/` — immutable AQ companion evidence; they do not replace the canonical reference, camera, SceneSpec, Visual QA, material, or production contracts.
19. `structural_migrations/<migration-id>/` — explicit SceneSpec V03 plan, candidate, derived copy, and receipt; never a canonical SceneSpec location.
20. `reports/v09/` — immutable privacy-safe environment probes and read-only workspace audit evidence.
21. `.cbm/queue/` — operational V0.9 single-worker queue state and immutable attempt receipts; never a canonical asset source.
22. `.blend`, renders, reports, bakes, and exports — derived artifacts; never edit them as the canonical fix.
...
The project version is `0.9.0`. The geometry SceneSpec contract remains `0.2.0` so existing v0.2 workspaces can be reused without rewriting approved geometry. Optional InteriorScope contracts use `0.1.0`, material contracts remain `0.5.0`, QA contracts remain `0.6.0`, portable static-asset contracts use `0.7.0`, orchestration contracts use `0.8.0`, and stabilization evidence uses `0.9.0`. AQ, Integrated Quality, Reference Evidence, MaterialGraph, Structural Geometry, and Assembly/Topology companion contracts use independent `0.1.0` versions; the derived-only SceneSpec V03 contract is `0.3.0` and never changes the default canonical SceneSpec version.
...
171. `client_mediated` production remains fail-closed until a supporting client binds the exact task and attests the exact restricted controller-tool-profile SHA-256. `desktop_in_session` rejects external task binding and begins from the current workflow boundary in the already-open Codex task.
172. `desktop_in_session` uses `approval_isolation=workflow_contract_only`. It does not attest a per-task MCP allowlist or shell restriction and must never be described as approval-isolated, sandboxed, or equivalent to an enforcing supporting client.
173. Desktop in-session convenience does not weaken any generic or specialized approval, exact fingerprint, single-writer lock, failed-retry boundary, bounded-convergence envelope, V0.7 optimization approval, Destination Handoff approval, or V0.9 postflight audit. An approval/retry surface may be used only after a new user message authorizes the exact current hash or failed step.
174. Controller execution mode is immutable dispatch evidence. Existing client-mediated or historical dispatches are never migrated, rebound, or reclassified into desktop in-session mode.
175. Autonomous Quality Extension `0.1.0` is an explicit opt-in production/controller overlay for a new `standard` static-prop workflow. It is not a third modeling pipeline, does not change the default `standard` or `background_exterior` policy, and does not promote the project beyond `0.9.0`.
176. `autonomous_static_prop_v1` accepts only a new `concept`, `primary_object_only`, explicitly targeted static prop with engine-neutral `portable_gltf` output. Interior, measured/blueprint/constraint work, rigging, skinning, animation, gameplay, destination-project writes, external network providers, CAD/B-Rep claims, and arbitrary Python/Blender/node execution are outside that profile.
177. An AQ `RootAuthorization` binds the exact initial request, primary-reference hash, job/workflow/dispatch, immutable profile and budget, content scope, target subject, output profile, allowed gates, and prohibited scopes. A `PolicyAuthorization` is a single-use decision by `autonomy_policy_engine` with `authorization_source=preauthorized_profile`; it is never represented as `approved_by=user` and never replaces InteriorScope, interior-camera, destination-import, reference/scope/target-change, budget-expansion, or arbitrary-code approval.
178. AQ budgets are immutable and cannot expand during execution. Candidate, structural, parametric, material, package-repair, Blender-build, quality-evaluation, canonical-promotion, plateau, repeated-failure, and global-action limits must terminate or route to review when exhausted; duplicate, A-B-A, A-B-C-B, repeated-direction, plateau, and repeated-failure detection must prevent unbounded work.
179. AQ candidates remain workflow-owned until exact host evaluation and one policy-authorized atomic promotion. Preserve the best-known candidate, rank hard-gate status before regressions, meaningful gain, Pareto dominance, changed-path count, change magnitude, and stable ID, and never promote a non-improving or regressing candidate merely because one weighted score is higher.
180. Integrated Quality `0.1.0` is a companion to Visual QA `0.6.0`; it preserves the exact legacy direct score while evaluating reference alignment, structural integrity, material fidelity, and production readiness separately. Missing required evidence is `unscorable`, never zero or pass, and machine JSON plus exact provenance remains authoritative over PDF projections.
181. AQ may create a `quality_passed` terminal only when the final Integrated Quality evidence is accepted and an immutable V0.7 `portable_gltf` package plus a fresh passed clean-import round trip agree on exact package, source, dependency, bounds, semantic, and material evidence. Optional handoff-envelope planning never writes to a destination project or proves runtime parity.
182. AQ package repair is a bounded derived-only exception, not a general retry. It may handle only an existing immutable package-ID collision or a roundtrip whose failed categories are exclusively `format`, must use a fresh `-aqrNN` package ID, record exact failure/plan/attempt/receipt evidence, preserve the canonical source fingerprint, and accept the result only after a fresh passed clean-import round trip. Material, bounds, dependency, Blender, unknown, stale, or tampered failures remain fail-closed.
183. AQ quality non-pass, unscorable evidence, or bounded quality termination produces an immutable review-only bundle when safe. Its manifest and receipt must state `production_ready=false` and `destination_handoff_eligible=false`; a review bundle is never a production package, clean-import acceptance, or Destination Handoff source.
184. SceneSpec V03 `0.3.0` is a parallel opt-in structural contract. Public migration plan/apply CLI and MCP surfaces require an exact plan SHA-256 and publish only run-owned derived copies and receipts under `structural_migrations/`; they must never replace canonical `analysis/scene_spec.json`, auto-migrate a legacy job, or make V03 the default AQ candidate contract.
185. AQ terminal verification must re-hash the final IQ JSON/manifest/PDF/profile/provenance and the exact nested package/roundtrip or review-bundle evidence. `quality_passed` and `review_required` evidence are mutually exclusive, convenience pointers are not freshness authority, and passing pre-production IQ alone cannot authorize either terminal artifact.
186. Existing workflows, jobs, approvals, receipts, blocked states, packages, and legacy SceneSpec `0.2.0` evidence remain readable and are never rewritten, retried, reclassified, or migrated by enabling AQ. AQ lifecycle and package-repair behavior applies only to newly planned AQ sessions.
187. Before any authorized side effect, a newly persisted AQ PolicyAuthorization must be reloaded and fully revalidated against the exact root, profile, budget, gate target, dependency hashes, predecessor chain, single-use state, and authorization-file hash identity. First-use persistence is not a validation exemption.
188. An AQ candidate owns its build, inventory, validation, modeling-plan, effective SceneSpec, assembly, topology, and quality provenance. Candidate selection must evaluate the candidate-owned evidence through the same hard-gate, Pareto, minimum-gain, and deterministic tie-break rules used by the controller; unavailable material or production evidence remains `unscorable`.
189. AQ structural assignments may optionally carry a complete SceneSpec V03 `0.3.0` candidate. The host may materialize every structural object into candidate-owned recipe, mesh, receipt, and `.blend` evidence and compile one path-backed SceneSpec `0.2.0` candidate for the existing build path, but canonical SceneSpec remains unchanged until an exact policy-authorized promotion.
190. The verified `autonomous_static_prop_v1` profile permits at most two material rounds by default. Material candidates remain workflow-owned, use strict promotion, and cannot weaken the existing MaterialPlan/ShaderRecipe `0.5.0` or make MaterialGraphSpec `0.1.0` a mandatory canonical input.
191. Budget exhaustion, plateau, cycle, repeated direction, or repeated failure must preserve the best-known evidence and route to an exact `review_required` outcome when a safe review bundle can be formed. They must never fabricate `quality_passed`, expand a budget, or continue an unbounded loop.
192. Portable package and Destination Handoff directory hashing must enumerate the same package-relative recursive file set under Windows long-path handling. Expected generation/postflight hash parity preserves valid evidence; missing, added, escaped, stale, or tampered files remain fail-closed.
