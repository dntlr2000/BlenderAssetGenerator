...
uv run pytest -q --basetemp "$OUTPUT_ROOT/pt-focused" "${FOCUSED_TESTS[@]}"
uv run ruff check .
uv run cbm doctor

if [[ "$SKIP_FULL_REGRESSION" -eq 0 ]]; then
  uv run pytest --basetemp "$OUTPUT_ROOT/pt-full"
fi
...
if [[ "$RUN_BLENDER" -eq 1 ]]; then
  ...
  CBM_RUN_PORTABLE_LONG_PATH_BLENDER_SMOKE=1 \
    uv run pytest -q --basetemp "$OUTPUT_ROOT/pt-blender" \
...
uv "${BENCHMARK_ARGS[@]}"
git diff --check
printf 'Autonomous Quality 0.1.0 gates completed: %s\n' "$OUTPUT_ROOT"
